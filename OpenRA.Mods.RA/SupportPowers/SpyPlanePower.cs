...
namespace OpenRA.Mods.RA
{
	...
	class SpyPlanePower : SupportPower
	{
		...
		public override void Activate(Actor self, Order order, SupportPowerManager manager)
		{
			...
			{
				new CenterPositionInit(self.World.CenterOfCell(enterCell) + new WVec(WRange.Zero, WRange.Zero, altitude)),
				new OwnerInit(self.Owner),
				new FacingInit(self.World.FacingBetween(enterCell, order.TargetLocation, 0))
			});
