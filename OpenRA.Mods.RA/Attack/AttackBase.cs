		...
		public Order IssueOrder(Actor self, IOrderTargeter order, Target target, bool queued)
		{
			if (order is AttackOrderTargeter)
			{
				...
				case TargetType.Terrain:
					return new Order("Attack", self, queued) { TargetLocation = self.World.CellContaining(target.CenterPosition) };
				}
			}
		...
		class AttackOrderTargeter : IOrderTargeter
		{
			...
			bool CanTargetLocation(Actor self, CPos location, List<Actor> actorsAtLocation, TargetModifiers modifiers, ref string cursor)
			{
				...
				if (modifiers.HasModifier(TargetModifiers.ForceAttack))
				{
					...
					var targetRange = (self.World.CenterOfCell(location) - self.CenterPosition).HorizontalLengthSquared;
			...
			public bool CanTarget(Actor self, Target target, List<Actor> othersAtTarget, TargetModifiers modifiers, ref string cursor)
			{
				...
				case TargetType.Terrain:
					return CanTargetLocation(self, self.World.CellContaining(target.CenterPosition), othersAtTarget, modifiers, ref cursor);
				default:
					return false;
				}
			}
