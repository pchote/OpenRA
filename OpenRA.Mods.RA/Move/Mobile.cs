	...
	public class Mobile : IIssueOrder, IResolveOrder, IOrderVoice, IPositionable, IMove, IFacing, ISync, INotifyAddedToWorld, INotifyRemovedFromWorld, INotifyBlockingMove
	{
		...
		public Mobile(ActorInitializer init, MobileInfo info)
		{
			...
			if (init.Contains<LocationInit>())
			{
				...
				SetVisualPosition(self, init.world.CenterOfCell(fromCell) + MobileInfo.SubCellOffsets[fromSubCell]);
			}
		...
		public void SetPosition(Actor self, CPos cell)
		{
			...
			SetVisualPosition(self, self.World.CenterOfCell(fromCell) + MobileInfo.SubCellOffsets[fromSubCell]);
			FinishedMoving(self);
		}
		...
		public Activity MoveIntoWorld(Actor self, CPos cell)
		{
			...
			// Animate transition
			var to = self.World.CenterOfCell(cell);
