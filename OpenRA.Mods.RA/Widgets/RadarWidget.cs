...
namespace OpenRA.Mods.RA.Widgets
{
	public class RadarWidget : Widget
	{
		...
		public override void Draw()
		{
			...
			if (hasRadar)
			{
				var tl = CellToMinimapPixel(world.CellContaining(worldRenderer.Position(worldRenderer.Viewport.TopLeft)));
				var br = CellToMinimapPixel(world.CellContaining(worldRenderer.Position(worldRenderer.Viewport.BottomRight)));
		...
		void DrawRadarPings()
		{
			...
			foreach (var radarPing in radarPings.Pings.Where(e => e.IsVisible()))
			{
				...
				var pingCell = world.CellContaining(radarPing.Position);
