...
if [ "$(uname -s)" = "Darwin" ]; then
	...
	patch_config SDL2 "${SEARCHDIRS}" SDL2-CS.dll.config SDL2.so libSDL2-2.0.so.0
	patch_config OpenAL "${SEARCHDIRS}" OpenAL-CS.Core.dll.config soft_oal.so libopenal.so.1
	patch_config FreeType "${SEARCHDIRS}" OpenRA.Platforms.Default.dll.config freetype6.so libfreetype.so.6
fi
