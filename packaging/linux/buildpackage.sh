...
if [ $# -lt "2" ]; then
	echo "Usage: $(basename "$0") version targetplatform [outputdir]"
	exit 1
fi
...
if command -v curl >/dev/null 2>&1; then
	...
	curl -s -L -O https://github.com/AppImage/AppImageKit/releases/download/continuous/AppRun-${ARCH}
else
	wget -cq https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage
	wget -cq https://github.com/AppImage/AppImageKit/releases/download/continuous/AppRun-${ARCH}
fi
...
build_appimage() {
	...
	if [ -n "${GITHUB_REPOSITORY}" ]; then
		./appimagetool-x86_64.AppImage --no-appstream -u "zsync|https://master.openra.net/appimagecheck.zsync?mod=${MOD_ID}&channel=${UPDATE_CHANNEL}" "${APPDIR}" "${OUTPUTDIR}/${APPIMAGE}" --runtime-file "AppRun-${ARCH}"
		...
		./appimagetool-x86_64.AppImage --no-appstream "${APPDIR}" "${OUTPUTDIR}/${APPIMAGE}" --runtime-file "AppRun-${ARCH}"
	fi
