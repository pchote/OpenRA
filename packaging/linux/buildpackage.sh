...
build_appimage() {
	...
	# appimagetool only supports update metadata for x86_64 and i386
	if [ -n "${GITHUB_REPOSITORY}" ] && [ "${ARCH}" == "x86_64" ]; then
		./appimagetool-x86_64.AppImage --no-appstream -u "zsync|https://master.openra.net/appimagecheck.zsync?mod=${MOD_ID}&channel=${UPDATE_CHANNEL}" "${APPDIR}" "${OUTPUTDIR}/${APPIMAGE}"
