...
namespace OpenRA.Platforms.Default
{
	...
	public sealed class FreeTypeFont : IFont
	{
		...
		public FontGlyph CreateGlyph(char c, int size, float deviceScale)
		{
			...
			var metrics_width = Marshal.ReadIntPtr(IntPtr.Add(metrics, FreeType.MetricsWidthOffset)); // face->glyph->metrics.width
			var metrics_height = Marshal.ReadIntPtr(IntPtr.Add(metrics, FreeType.MetricsHeightOffset)); // face->glyph->metrics.width
			var metrics_advance = Marshal.ReadIntPtr(IntPtr.Add(metrics, FreeType.MetricsAdvanceOffset)); // face->glyph->metrics.horiAdvance

			var bitmap = IntPtr.Add(glyph, FreeType.GlyphSlotBitmapOffset); // face->glyph->bitmap
			var bitmap_pitch = Marshal.ReadInt32(IntPtr.Add(bitmap, FreeType.BitmapPitchOffset)); // face->glyph->bitmap.pitch
			var bitmap_buffer = Marshal.ReadIntPtr(IntPtr.Add(bitmap, FreeType.BitmapBufferOffset)); // face->glyph->bitmap.buffer

			var bitmap_left = Marshal.ReadInt32(IntPtr.Add(glyph, FreeType.GlyphSlotBitmapLeftOffset)); // face->glyph.bitmap_left
			var bitmap_top = Marshal.ReadInt32(IntPtr.Add(glyph, FreeType.GlyphSlotBitmapTopOffset)); // face->glyph.bitmap_top

			// metrics_width, metrics_height, metrics_advance are in FreeType's 26.6 fixed point format - discard fractional bits
			var glyphSize = new Size((int)metrics_width >> 6, (int)metrics_height >> 6);

			var g = new FontGlyph
			{
				Advance = (int)metrics_advance >> 6,
				Offset = new int2(bitmap_left, -bitmap_top),
				Size = glyphSize,
