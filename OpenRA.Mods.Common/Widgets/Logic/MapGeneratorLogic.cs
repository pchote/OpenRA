...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public class MapGeneratorLogic : ChromeLogic
	{
		...
		internal MapGeneratorLogic(Widget widget, ModData modData, Action<MapGenerationArgs> onGenerate)
		{
			...
			{
				var desc = new CachedTransform<Map, string>(m =>
				{
					var players = new MapPlayers(m.PlayerDefinitions);
					var details = FluentProvider.GetMessage(Players, "players", players.Players.Count(x => x.Value.Playable));
					if (m.Categories.Length > 0)
						details = m.Categories[0] + " " + details;

					return details;
				});

				previewDetailsLabel.IsVisible = () => !(generating || failed);
				previewDetailsLabel.GetText = () => desc.Update(generatedMap);
			}

			var previewAuthorLabel = widget.GetOrNull<LabelWithTooltipWidget>("AUTHOR");
			if (previewAuthorLabel != null)
			{
				var author = new CachedTransform<Map, string>(m => FluentProvider.GetMessage(CreatedBy, "author", m.Author));
				previewAuthorLabel.IsVisible = () => !(generating || failed);
				previewAuthorLabel.GetText = () => author.Update(generatedMap);
			}

			var previewSizeLabel = widget.GetOrNull<LabelWidget>("SIZE");
			if (previewSizeLabel != null)
			{
				var size = new CachedTransform<Map, string>(m => MapChooserLogic.MapSizeLabel(m.Bounds.Size));
				previewSizeLabel.IsVisible = () => !(generating || failed);
				previewSizeLabel.GetText = () => size.Update(generatedMap);
			}
