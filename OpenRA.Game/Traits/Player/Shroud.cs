...
namespace OpenRA.Traits
{
		...
		public Rectangle GetRenderBounds(Map map)
		{
			...
			if (VisibilityMargin.Length == 4)
			{
				...
				return Rectangle.FromLTRB(
					map.Bounds.Left - margin[0],
					map.Bounds.Top - margin[1],
					map.Bounds.Right + margin[2],
					map.Bounds.Bottom + margin[3]);
			}
			...
			if (VisibilityMargin.Length == 1)
			{
				...
				return Rectangle.FromLTRB(
					map.Bounds.Left - margin,
					map.Bounds.Top - margin,
					map.Bounds.Right + margin,
					map.Bounds.Bottom + margin);
			}
	...
	public class Shroud : ISync, INotifyCreated, ITick
	{
		...
		{
			// Note: This is a copy of Map.ContainsAllProjectedCellsCovering, but for the shroud's
			// custom renderBounds
			if (map.Grid.MaximumTerrainHeight == 0)
				return renderBounds.Contains(uv.U, uv.V);
