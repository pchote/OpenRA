...
{
	class ImportGen2MapCommand : IUtilityCommand
	{
		public string Name { get { return "--import-gen2-map"; } }
		...
		void UnpackLZO(byte[] src, byte[] dest)
		{
			...
			while (destOffset < dest.Length)
			{
				...
				srcOffset += 4;
				MiniLZO.DecodeInto(src, srcOffset, srcLength, dest, destOffset, ref destLength);
				srcOffset += srcLength;
		...
		void UnpackLCW(byte[] src, byte[] dest, byte[] temp)
		{
			...
			while (destOffset < dest.Length)
			{
				...
				srcOffset += 4;
				Format80.DecodeInto(src, temp, srcOffset);
