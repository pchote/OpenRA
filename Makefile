...
all:
	...
	@$(MSBUILD) -t:Build -restore -p:Configuration="Release - Mono" -p:TargetPlatform=$(TARGETPLATFORM) -p:TargetFramework=netstandard2.1 -p:Mono=true -p:DefineConstants="MONO"
...
check:
	...
	@$(MSBUILD) -t:build -restore -p:Configuration="Debug - Mono" -p:TargetPlatform=$(TARGETPLATFORM) -p:TargetFramework=netstandard2.1 -p:Mono=true -p:DefineConstants="MONO"
	@echo
